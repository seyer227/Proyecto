...
def logs():
    return rx.vstack(
        rx.heading("historial", size="6"),
        rx.foreach(
            Foo.logs,
            lambda log: rx.text(log, color="gray", size="3"),
        ),
        rx.spacer(),
        rx.button("limpiar historial", on_click=Foo.clear_logs, size="3", color_scheme="ruby"),
        align="center",
        width="50%",
        height="100%",
        spacing="1",
...
@rx.page(route= Route.CALENDARIO.value, title='Login')
